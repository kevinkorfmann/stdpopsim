...
1 early() {
    ...
    // For each stdvoidsim mutation type with an h-s relationship
...
_raw_stdvoidsim_top_level_schema = {
    "stdvoidsim": {
...
def _add_dfes_to_metadata(ts, contig):
    ...
    schema["properties"].update(_raw_stdvoidsim_top_level_schema)
    ...
    new_metadata = {"stdvoidsim": {"DFEs": dfes}}
...
def slim_makescript(
    ...
    def fix_time(event):
        for attr in ("time", "start_time", "end_time"):
            ...
            if isinstance(t, stdvoidsim.GenerationAfter):
    ...
    op_id = stdvoidsim.ConditionOnAlleleFrequency.op_id
    ...
    for ee in extended_events:
        ...
        if hasattr(ee, "start_time") and hasattr(ee, "end_time"):
            ...
            stdvoidsim.validate_time_range(ee.start_time, ee.end_time)
        ...
        if isinstance(ee, stdvoidsim.DrawMutation):
        ...
        elif isinstance(ee, stdvoidsim.ChangeMutationFitness):
        ...
        elif isinstance(ee, stdvoidsim.ConditionOnAlleleFrequency):
            assert population_id >= 0
    ...
    # Header
    printsc("/*")
    printsc(" * stdvoidsim " + stdvoidsim.__version__)
    printsc(" *")
    ...
    for j, d, ints in _enum_dfe_and_intervals(contig):
        ...
        if ints.shape[0] > 0:
            ...
            # stdvoidsim intervals are 0-based left inclusive, right exclusive, but
    ...
    op_types = ", ".join(
        f'"{op}"' for op in stdvoidsim.ConditionOnAlleleFrequency.op_types
    )
    ...
    for sample_set in samples:
        ...
        if sample_set.ploidy == 1:
            ...
            if count % 2 != 0:
                ...
                warnings.warn(
                    stdvoidsim.SLiMOddSampleWarning(
...
class _SLiMEngine(stdvoidsim.Engine):
    id = "slim"  #:
    ...
    citations = [
        stdvoidsim.Citation(
            ...
            year=2019,
            author="Haller et al.",
            reasons={stdvoidsim.CiteReason.ENGINE},
        ),
    ]
    ...
    def simulate(
        ...
        * ``separate_sexes``: if the species has separate sexes, SLiM
        ...
        if slim_scaling_factor != 1:
            warnings.warn(
                stdvoidsim.SLiMScalingFactorWarning(
        ...
        def _slim_tempdir():
            tempdir = tempfile.TemporaryDirectory(
                prefix="stdvoidsim_", ignore_cleanup_errors=True
            )
        ...
        with _slim_tempdir() as st:
            ...
            if contig.inclusion_mask is not None:
                ts = stdvoidsim.utils.mask_tree_sequence(
                    ts, contig.inclusion_mask, False
                )
            if contig.exclusion_mask is not None:
                ts = stdvoidsim.utils.mask_tree_sequence(ts, contig.exclusion_mask, True)
    ...
    def _run_slim(
        ...
        with subprocess.Popen(
            ...
            try:
                ...
                warnings.warn(
                    stdvoidsim.UnspecifiedSLiMWarning(
            ...
            for line in outs.splitlines():
                ...
                if line.startswith("WARNING: "):
                    warnings.warn(
                        stdvoidsim.UnspecifiedSLiMWarning(line[len("WARNING: ") :])
                    )
                else:
    ...
    def _recap_and_rescale(
        ...
        if extended_events is not None:
            ...
            for dfe in metadata["stdvoidsim"]["DFEs"]:
        ...
        metadata["stdvoidsim"]["DFEs"].append(recap_dfe)
        tables.metadata = metadata
        ...
        if contig.ploidy == 1:
            ts = stdvoidsim.utils.haploidize_individuals(ts)
    ...
    def recap_and_rescale(
        ...
        **kwargs,
        ...
        run outside of stdvoidsim, this function can be used to transform the
        SLiM tree sequence following the procedure that would have been used
        if stdvoidsim had run SLiM itself.
        ...
        .. warning::
            ...
            make to the SLiM code before using it, the stdvoidsim source code
...
stdvoidsim.register_engine(_SLiMEngine())
