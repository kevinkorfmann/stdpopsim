"""
The command line interface for stdvoidsim (fork of stdvoidsim).
...
def get_species_wrapper(species_id):
    try:
        return stdvoidsim.get_species(species_id)
...
def get_environment():
    """
    Returns a dictionary describing the environment in which stdvoidsim
...
def get_provenance_dict():
    """
    Returns a dictionary encoding an execution of stdvoidsim conforming to the
    tskit provenance schema.
    """
    document = {
        "schema_version": "1.0.0",
        "software": {"name": "stdvoidsim", "version": stdvoidsim.__version__},
...
def get_citations(engine, model, contig, species, dfe):
    ...
    return stdvoidsim.Citation.merge(citations)
...
def write_citations(engine, model, contig, species, dfe):
    ...
    for citation in get_citations(engine, model, contig, species, dfe):
        if (
            stdvoidsim.citations.CiteReason.MUT_RATE in citation.reasons
            and model.mutation_rate is not None
        ):
            continue
        if (
            stdvoidsim.citations.CiteReason.REC_RATE in citation.reasons
...
def add_simulate_species_parser(parser, species):
    ...
    if len(species.genetic_maps) > 0:
        species_parser.add_argument(
            ...
            help=(
                ...
                "the catalog: "
                "<https://popsim-consortium.github.io/stdvoidsim-docs/"
                "stable/catalog.html> "
                "Available maps: "
                f"{', '.join(choices)}. "
            ),
    ...
    def run_simulation(args):
        ...
        samples = stdvoidsim.utils.parse_population_sample_pairs(args.samples)
        ...
        engine = stdvoidsim.get_engine(args.engine)
        logger.info(
        ...
        if not qc_complete:
            warnings.warn(
                stdvoidsim.QCMissingWarning(
                    f"{model.id} has not been QCed. Use at your own risk! "
                    "Demographic models that have not undergone stdvoidsim's "
...
def run_download_genetic_maps(args):
    ...
    if args.species is None:
        species_names = [species.id for species in stdvoidsim.all_species()]
...
def stdvoidsim_cli_parser():
    ...
    top_parser.add_argument(
        ...
        "--version",
        action="version",
        version="%(prog)s {}".format(stdvoidsim.__version__),
    )
    ...
    top_parser.add_argument(
        ...
        help=(
            ...
            f"Default: {stdvoidsim.get_cache_dir()}"
        ),
    )
    ...
    top_parser.add_argument(
        "-e",
        "--engine",
        default=stdvoidsim.get_default_engine().id,
        choices=[e.id for e in stdvoidsim.all_engines()],
        help="Specify a simulation engine.",
    )

    supported_models = stdvoidsim.get_engine("msprime").supported_models
...
def stdvoidsim_main(arg_list=None):
    parser = stdvoidsim_cli_parser()
    args = parser.parse_args(arg_list)
    setup_logging(args)
    if args.cache_dir is not None:
        stdvoidsim.set_cache_dir(args.cache_dir)
    run(args)
