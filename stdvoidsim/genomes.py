...
class Contig:
    ...
    .. note::
        To run stdvoidsim simulations with alternative, user-specified mutation,
    ...
    def __attrs_post_init__(self):
        ...
        if self.species is None:
            self.species = stdvoidsim.species.Species(
                id="default",
    ...
    def species_contig(
        *,
        ...
        if chromosome is not None and chromosome.lower() in non_autosomal_lower:
            warnings.warn(
                stdvoidsim.NonAutosomalWarning(
                    "Non-autosomal simulations are not yet supported. See "
                    "https://github.com/popsim-consortium/stdvoidsim/issues/383 and "
                    "https://github.com/popsim-consortium/stdvoidsim/issues/406"
                )
            )
        if (
        ...
        if length_multiplier is None:
            ...
            warnings.warn(
                stdvoidsim.DeprecatedFeatureWarning(
        ...
        if chromosome is None:
            ...
            if inclusion_mask is not None:
                ...
                if isinstance(inclusion_mask, str):
                    inclusion_intervals = stdvoidsim.utils.read_bed(
                        inclusion_mask, chromosome
                    )
                else:
                    inclusion_intervals = inclusion_mask
                inclusion_intervals = stdvoidsim.utils.clip_intervals(
            ...
            if exclusion_mask is not None:
                ...
                if isinstance(exclusion_mask, str):
                    exclusion_intervals = stdvoidsim.utils.read_bed(
                        exclusion_mask, chromosome
                    )
                else:
                    exclusion_intervals = exclusion_mask
                exclusion_intervals = stdvoidsim.utils.clip_intervals(
    ...
    def add_dfe(self, intervals, DFE):
        ...
        """
        _, left, right = self.coordinates
        intervals = stdvoidsim.utils.clip_intervals(intervals, left, right)
        stdvoidsim.utils._check_intervals_validity(intervals, start=left, end=right)
        for j, ints in enumerate(self.interval_list):
            self.interval_list[j] = stdvoidsim.utils.mask_intervals(ints, intervals)
    ...
    def add_single_site(
        ...
        mut_type = stdvoidsim.MutationType(
            distribution_type="f",
            dominance_coeff=0.5,
