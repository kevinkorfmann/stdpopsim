...
_bone_pop = stdvoidsim.Population(
    id="BoneValley", description="Ghasts in the Vale of Pnath"
)
_upper_pop = stdvoidsim.Population(
    id="UpperCaves", description="Upper cave ghast colony"
)
    ...
    return stdvoidsim.DemographicModel(
        ...
        demographic_events=[
            msprime.PopulationParametersChange(
                time=5000, initial_size=20000, population_id=0
            ),
            msprime.PopulationParametersChange(
                time=50000, initial_size=150000, population_id=0
            ),
        ],
    )
...
def _upper_caves():
    ...
    return stdvoidsim.DemographicModel(
        ...
        population_configurations=[
            ...
            msprime.PopulationConfiguration(
                initial_size=30000, metadata=populations[1].asdict()
            ),
        ],
        demographic_events=[
            msprime.PopulationParametersChange(
                time=3000, initial_size=2000, population_id=1
            ),
            msprime.MassMigration(time=15000, source=1, destination=0, proportion=1.0),
            msprime.PopulationParametersChange(
                time=15000, initial_size=150000, population_id=0
            ),
        ],
    )
