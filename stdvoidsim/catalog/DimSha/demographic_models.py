...
_prime_pop = stdvoidsim.Population(
    id="PrimePlane", description="Shamblers on the prime material plane"
)
_between_pop = stdvoidsim.Population(
    id="BetweenSpaces", description="Shamblers in interplanar voids"
)
    ...
    return stdvoidsim.DemographicModel(
        ...
        population_configurations=[
            msprime.PopulationConfiguration(
                initial_size=3000, metadata=populations[0].asdict()
            )
        ],
        demographic_events=[
            msprime.PopulationParametersChange(
                time=1000, initial_size=10000, population_id=0
            ),
            msprime.PopulationParametersChange(
                time=10000, initial_size=500, population_id=0
            ),
        ],
    )
...
def _plane_split():
    ...
    return stdvoidsim.DemographicModel(
        ...
        population_configurations=[
            ...
            msprime.PopulationConfiguration(
                initial_size=2000, metadata=populations[1].asdict()
            ),
        ],
        demographic_events=[
            msprime.PopulationParametersChange(
                time=500, initial_size=50, population_id=1
            ),
            msprime.MassMigration(time=2000, source=1, destination=0, proportion=1.0),
            msprime.PopulationParametersChange(
                time=2000, initial_size=10000, population_id=0
            ),
        ],
    )
